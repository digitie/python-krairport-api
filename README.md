...
`pykrairport`는 "인천공항은 인천국제공항공사, 그 외 전국공항은 한국공항공사"라는 공급자 경계를 내부에서 흡수하고, 운항/주차/혼잡도 데이터를 Python 타입과 dataclass 모델로 일관되게 제공하는 것을 목표로 합니다.
...
```python
from pykrairport import KrairportClient

airport = KrairportClient.from_env()

# 1) 김포공항 출발편 조회 -> KAC로 자동 라우팅
for flight in airport.departures(airport_code="GMP", searchday="20260430", from_time="0600", to_time="1200"):
