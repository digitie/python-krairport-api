...
- 공항 좌표는 `krairport.Coordinate`, 시설 주소는 `str | None`을 파라미터와 반환 모델에 사용합니다.
- KAC는 XML-heavy, IIAC는 JSON 우선 호출을 기본으로 합니다.
- 런타임 의존성은 `httpx`, `pydantic`, Windows용 `tzdata`입니다.
...
- 불필요한 wrapper나 호환층을 새로 만들지 않습니다. 좌표와 주소 처리는 `krairport` 내부 구현으로 유지하고 외부 주소/좌표 패키지에 의존하지 않습니다.
...
- 좌표 정규화, DMS 파싱, 거리 계산은 `src/krairport/geo.py`의 `Coordinate`와 helper를 사용합니다.
- 주소는 provider row의 실제 주소 필드가 있을 때만 문자열로 보존합니다.
...
- 좌표 순서를 섞지 않습니다. `Coordinate.as_tuple()`과 `as_lat_lon()`은 `(latitude, longitude)`, `as_geojson_position()`은 `(longitude, latitude)`입니다.
- 공항 내부 위치 문자열을 주소로 추정하지 않습니다. provider row에 주소 필드가 있을 때만 문자열 주소를 보존합니다.
...
- 좌표/주소는 `src/krairport/geo.py` 안에서 처리하며 외부 주소/좌표 패키지를 다시 도입하지 않습니다.
